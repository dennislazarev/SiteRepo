...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h2>Список прав сотрудников</h2>
    <?php if (\App\Core\Auth::can('permission_create')): ?>
    <a href="/admin/permissions/create" class="btn btn-primary"><i class="fas fa-plus-circle"></i> Создать право</a>
    <?php endif; ?>
</div>

<?php if (isset($permissions) && is_array($permissions) && count($permissions) > 0): ?>
    <div class="table-responsive">
        <table class="table table-striped table-hover align-middle filterable-table" id="permissions-table">
            <thead class="table-dark">
                <tr class="text-center">
                    ...
                    <th scope="col" data-filterable="module_select">Модуль</th>
                ...
                $tableHeaders = [
                    ...
                    ['text' => 'Модуль', 'filterable' => 'module_select'],
                    ['text' => 'Системное', 'filterable' => 'select'],
                    ['text' => 'Действия', 'filterable' => 'false'],
                ];
                $tableId = 'permissions-table'; // Должен совпадать с ID таблицы
            ...
            <tbody class="text-center">
                ...
                <tr>
                    ...
                    <td class="actions-cell">
                        <?php if (\App\Core\Auth::can('permission_edit')): ?>
                            <a href="/admin/permissions/<?= $perm['id'] ?>/edit" class="btn btn-sm btn-actions btn-outline-primary me-1" title="Редактировать">
                                <i class="fas fa-edit"></i>
                            </a>
                        <?php endif; ?>
                        <?php if (\App\Core\Auth::can('permission_delete') && !$perm['is_system']): ?>
                            <button type="button" class="btn btn-actions btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#deletePermissionModal"
...
<?php else: ?>
    <div class="alert alert-info text-center">
        ...
        <?php if (!empty(array_filter($_GET))): ?>
            <a href="/admin/permissions" class="alert-link">Сбросить фильтры</a>.
        <?php endif; ?>
    </div>
<?php endif; ?>

<!-- Подключение универсального модального окна подтверждения удаления для Прав -->
<?php
    $modalId = 'deletePermissionModal';
    $title = 'Подтвердите удаление права';
    $itemNameDisplay = 'delete-permission-name'; // Имя переменной для отображения в модальном окне
    $formActionBase = 'delete-permission';     // Базовая часть action формы
